...
// --- 自動載入 Tailwind CSS (解決線上環境預設無樣式的問題) ---
if (
...
// 加入防呆機制：使用 try-catch 包覆，避免金鑰不完整導致系統完全崩潰 (Cannot read properties of undefined 'default')
let app, auth, db;
...
try {
  ...
  console.error(
    "Firebase 初始化失敗！請檢查 firebaseConfig 是否填寫完整:",
    error
  );
}
...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsubUsers = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.empty) {
          ...
          setDoc(doc(usersRef, "admin_init"), {
            id: "admin_init",
            username: "admin",
            password: "password",
            name: "系統管理員",
          ...
          setDoc(doc(usersRef, "employee_init"), {
            id: "employee_init",
            username: "employee",
            password: "password",
            name: "測試員工小明",
    ...
    const unsubRecords = onSnapshot(
      recordsRef,
      (snapshot) => {
        const loadedRecords = snapshot.docs.map((d) => ({
          ...d.data(),
          docId: d.id,
        }));
        loadedRecords.sort(
  ...
  const handlePunch = async (type) => {
    ...
    const hasPunchedToday = records.some(
      (r) =>
  ...
  const handleUpdateUserIP = async (docId, newIP) => {
    try {
      await updateDoc(
        doc(db, "artifacts", appId, "public", "data", "punch_users", docId),
        { allowedIP: newIP }
      );
      showToast(
        newIP ? `已更新 IP 限制規則` : "已解除該員工的 IP 限制",
        "success"
      );
    } catch (err) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        ...
        <p className="text-gray-600 text-center max-w-lg leading-relaxed bg-white p-6 rounded-xl shadow-sm border border-gray-200 mt-4">
          ...
          <br />
          <br />
          👉 請回到 Firebase 控制台，複製<strong>完整</strong>的設定碼（包含
          apiKey, projectId,{" "}
          <span className="text-red-600 font-bold">appId</span>{" "}
          等所有欄位），並貼上覆蓋程式碼中的設定區塊。
        </p>
      </div>
    );
  }
  ...
  if (!isDataLoaded) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center p-4">
        ...
        <h2 className="text-xl font-bold text-gray-800 tracking-wide animate-pulse">
          正在連線至雲端資料庫...
        </h2>
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col font-sans">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
          <div className="flex items-center space-x-2 text-blue-600 flex-shrink-0">
            ...
            <h1 className="font-bold text-lg sm:text-xl tracking-wide truncate hidden xs:block">
              企業打卡系統{" "}
              <span className="text-[10px] text-emerald-600 border border-emerald-500 bg-emerald-50 px-1.5 py-0.5 rounded-md ml-1 relative -top-0.5">
                雲端測試版
              </span>
            </h1>
          </div>
      ...
      {toast && (
        <div className="fixed top-20 left-4 right-4 sm:left-1/2 sm:right-auto sm:transform sm:-translate-x-1/2 z-50 animate-fade-in-down flex justify-center">
          <div
            className={`flex items-center w-full sm:w-auto px-4 sm:px-5 py-3 rounded-lg shadow-xl ${
              toast.type === "success"
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-gray-200 flex items-center justify-center p-4">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
        <div className="bg-blue-600 p-6 sm:p-8 text-center relative overflow-hidden">
          ...
          <h2 className="text-2xl sm:text-3xl font-bold text-white tracking-wider relative z-10">
            打卡系統登入
          </h2>
          <p className="text-blue-100 mt-1 sm:mt-2 text-xs sm:text-sm relative z-10">
        ...
        <div className="p-6 sm:p-8">
          {toast && (
            <div
              className={`mb-6 p-3 sm:p-4 rounded-lg flex items-center text-sm ${
                toast.type === "error"
          ...
          <form onSubmit={handleSubmit} className="space-y-5 sm:space-y-6">
            ...
            <div className="flex items-center">
              ...
              <label
                htmlFor="remember-me"
                className="ml-2 block text-sm sm:text-base text-gray-700 cursor-pointer select-none"
              >
                記住帳號與密碼
              </label>
            </div>
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 sm:p-12 text-center relative overflow-hidden">
        ...
        <h2 className="text-lg sm:text-xl text-gray-500 mb-2 sm:mb-3 mt-2 sm:mt-4">
              ...
              <ul className="divide-y divide-gray-100">
                {todayRecords.map((record) => (
                  ...
                  >
                    <div className="flex items-center">
                      <span
                        className={`w-2.5 h-2.5 sm:w-3 sm:h-3 rounded-full mr-3 sm:mr-4 ${
                          record.type === "in"
                            ? "bg-blue-500"
                            : "bg-emerald-500"
                        }`}
              ...
              <ul className="divide-y divide-gray-100">
                {records.map((record) => {
                  ...
                  return (
                    <li
                      key={record.docId}
                      className="px-4 sm:px-6 py-3 sm:py-4 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-gray-50 gap-1 sm:gap-0"
                    >
                      <div className="flex items-center">
                        <span
                          className={`px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[10px] sm:text-xs font-bold mr-3 sm:mr-4 ${
                            record.type === "in"
...
function AdminRecordsView({ records, users, showToast }) {
  ...
  const handleExportCSV = () => {
    ...
    try {
      ...
      let dateStr = "全部";
      if (startDate && endDate)
        dateStr = `${startDate.replace(/-/g, "")}-${endDate.replace(/-/g, "")}`;
      ...
      <div className="bg-blue-50/50 border border-blue-100 rounded-xl p-4 sm:p-5 mb-6 flex flex-col lg:flex-row gap-4 sm:gap-5">
        ...
        <div className="flex-1 w-full">
          <label className="block text-sm font-bold text-gray-700 mb-1.5">
            ...
            <button
              ...
              className="ml-2 text-xs text-blue-600 font-normal hover:underline"
            >
              [ 今天 ]
            </button>
          </label>
        ...
        <div className="flex gap-3 sm:gap-4 flex-1 w-full lg:max-w-[400px]">
          <div className="flex-1 w-full min-w-0">
            <label className="block text-sm font-bold text-gray-700 mb-1.5 truncate">
              依員工查詢
            </label>
            <div className="relative">
              <select
          ...
          <table className="w-full text-left border-collapse bg-white min-w-[500px]">
            <thead>
              <tr className="bg-gray-100/80 text-gray-700 text-xs sm:text-sm uppercase tracking-wider border-b border-gray-200">
                <th className="p-3 sm:p-4 font-bold whitespace-nowrap">
                  員工姓名
                </th>
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredRecords.map((record) => {
                ...
                return (
                  ...
                  >
                    <td className="p-3 sm:p-4 font-bold text-gray-900 text-sm sm:text-base whitespace-nowrap">
                      {record.userName}
                    </td>
                    <td className="p-3 sm:p-4 whitespace-nowrap">
                      <span
                        className={`inline-flex items-center px-2 py-0.5 sm:px-3 sm:py-1 rounded-full text-xs sm:text-sm font-bold shadow-sm ${
                          record.type === "in"
                    ...
                    <td className="p-3 sm:p-4 text-gray-700 text-sm sm:text-base whitespace-nowrap">
                      {dateObj.toLocaleDateString("zh-TW")}
                    </td>
                    <td className="p-3 sm:p-4 text-gray-700 font-mono text-sm sm:text-lg whitespace-nowrap">
                      {dateObj.toLocaleTimeString("zh-TW")}
                    </td>
                  </tr>
                );
...
function AdminUsersView({
  ...
  const startEditIp = (user) => {
    setEditingIpUserId(user.docId);
    setEditIpValue(user.allowedIP || "");
  };
  const saveEditIp = (docId) => {
    onUpdateUserIP(docId, editIpValue.trim());
          ...
          <div className="bg-white rounded-2xl p-5 sm:p-6 max-w-sm w-full shadow-2xl animate-fade-in-down">
            ...
            <div className="flex justify-end gap-2 sm:gap-3">
              <button
                ...
                className="px-4 sm:px-5 py-2 sm:py-2.5 text-gray-600 font-bold hover:bg-gray-100 rounded-lg text-sm sm:text-base"
              >
                取消
              </button>
              <button
                ...
                className="px-4 sm:px-5 py-2 sm:py-2.5 bg-red-600 text-white font-bold hover:bg-red-700 rounded-lg shadow-md text-sm sm:text-base"
              >
                確定刪除
              </button>
            </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
          <ul className="divide-y divide-gray-100">
            {users.map((user) => (
              <li
                key={user.docId}
                className="p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between hover:bg-gray-50 transition-colors gap-3 sm:gap-4"
              >
                <div className="flex items-start sm:items-center gap-3 sm:gap-4 w-full sm:w-auto">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-white shadow-sm flex-shrink-0 mt-1 sm:mt-0 ${
                  ...
                  <div className="flex-1 min-w-0 w-full">
                    <div className="font-bold text-gray-900 text-base sm:text-lg flex flex-wrap items-center gap-2">
                      ...
                      {user.id === currentUser.id && (
                        <span className="text-[10px] sm:text-xs bg-gray-200 text-gray-600 px-2 py-0.5 rounded-full flex-shrink-0">
                          您自己
                        </span>
                      )}
                    </div>
                      ...
                      {editingIpUserId === user.docId ? (
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-1.5 sm:ml-2 w-full">
                          <input
                            ...
                            placeholder="支援單一,區間(-),萬用(*)"
                            autoFocus
                            onKeyDown={(e) => {
                              if (e.key === "Enter") saveEditIp(user.docId);
                              if (e.key === "Escape") setEditingIpUserId(null);
                            }}
                          />
                          <div className="flex gap-1.5">
                            <button
                              onClick={() => saveEditIp(user.docId)}
                              className="flex-1 sm:flex-none text-white hover:bg-green-600 bg-green-500 px-2 py-1 rounded shadow-sm font-bold text-xs text-center"
                            >
                              儲存
                            </button>
                            <button
                              onClick={() => setEditingIpUserId(null)}
                              className="flex-1 sm:flex-none text-gray-600 hover:bg-gray-300 bg-gray-200 px-2 py-1 rounded shadow-sm font-bold text-xs text-center"
                            >
                              取消
                            </button>
                          </div>
                        ...
                        <div className="flex items-center ml-1 sm:ml-0 mt-1 sm:mt-0">
                          {user.allowedIP ? (
                            <span
                              className="sm:ml-1 text-blue-600 font-mono font-bold truncate max-w-[120px] sm:max-w-xs"
                              title={user.allowedIP}
                            >
                              {user.allowedIP}
                            </span>
                          ) : (
                          ...
                          <button
                            onClick={() => startEditIp(user)}
                            className="ml-2 text-blue-500 hover:text-blue-700 p-1 sm:p-0 bg-blue-50 sm:bg-transparent rounded"
                            title="設定登入IP"
                          >
                ...
                <div className="flex items-center justify-end gap-3 w-full sm:w-auto mt-2 sm:mt-0 pt-3 sm:pt-0 border-t sm:border-0 border-gray-100">
                  <span
                    className={`text-[10px] sm:text-xs px-2.5 sm:px-3 py-1 rounded-full font-bold shadow-sm ${
                      user.role === "admin"
          ...
          >
            ...
            <div>
              ...
              <input
                ...
                placeholder="例: 192.168.1.1-192.168.1.50, 10.0.*"
            ...
            <button
              type="submit"
              className="w-full mt-2 sm:mt-4 bg-blue-600 hover:bg-blue-700 text-white font-bold py-2.5 sm:py-3 px-4 rounded-lg transition-all shadow-md active:scale-95 flex justify-center items-center text-sm sm:text-base"
            >
